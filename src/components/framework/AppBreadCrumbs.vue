<template>
  <div class="app-bread-crumbs">
    <span v-for="(item, index) in breadCrumbsList" :key="index">
      <template v-if="index + 1 !== breadCrumbsList.length">
...
export default {
  ...
  computed: {
    breadCrumbsList() {
      ...
      matched.filter(item => item && item.path && item.name && item.path !== '/').forEach((item) => {
